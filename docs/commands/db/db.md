...
- `--replica` поддерживается только в Yandex-контекстах. Список доступных реплик CLI проверяет сам через служебный dry-run запрос к оператору — пользователю ничего дополнительно запускать не нужно.
